// ═══════════════════════════════════════════════════════════════
// LIVEVIEW_ENHANCEMENTS.JS v3.0 — Sempurna
// Perbaikan:
//   1. AT_LIVE_SYNC  — instant sync via input event + fallback polling
//   2. AT_LAYOUT     — layout picker dengan injeksi CSS nyata ke student HTML
//   3. Page select sync dua arah (split ↔ preview panel)
//   4. Live pill yang akurat
// ═══════════════════════════════════════════════════════════════
...
window.AT_LIVE_SYNC = {
  ...
  init() {
    this._pill = document.getElementById('liveSyncPill');
    this._installListeners();
    // Polling fallback: tiap 1.5 detik cek apakah state berubah (untuk field yang
    // mungkin tidak trigger event, misal programmatic set)
    this._lastStateStr = '';
    setInterval(() => this._pollState(), 1500);
    console.log('✅ AT_LIVE_SYNC v3 ready');
  },

  _installListeners() {
    // Event delegation di level document — menangkap SEMUA field
    // termasuk yang di-render secara dinamis (TP, ATP, Alur, Kuis, Modul, dll)
    const capture = true; // capture phase agar tidak terlewat

    document.addEventListener('input', (e) => {
      const tag = e.target.tagName;
      if (!['INPUT','TEXTAREA'].includes(tag)) return;
      if (e.target.closest('#split-pane')) return; // jangan trigger dari preview frame
  ...
  scheduleSync(reason) {
    if (!AT_SPLITVIEW?.active) return;
    this._setPillState('syncing');
    clearTimeout(this._timer);
    this._timer = setTimeout(() => {
      AT_SPLITVIEW.refresh();
      this._setPillState('live');
    }, this._delay);
  },

  _setPillState(state) {
    if (!this._pill) return;
    this._pill.classList.toggle('syncing', state === 'syncing');
  },

  onSplitToggle(active) {
    if (!this._pill) return;
    this._pill.style.display = active ? 'inline-flex' : 'inline-flex';
    this._pill.style.opacity = active ? '1' : '0.4';
    if (active) {
      // Reset poll baseline
      this._lastStateStr = '';
    }
...
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => {
    ...
    console.log('✅ liveview_enhancements.js v3.0 — semua sistem aktif');
  }, 150);
});
